...
Output structure:
  Predict_ROI_2_6_1_2_All/
  ├── 01_PCA_Analysis/
  ├── 02_Model_Linear_PC1/
  ├── 03_Model_Linear_PC1_PC2/
  ├── 04_Model_Poly_Deg2/
  ├── 05_Model_SVR/
  ├── 06_Model_GPR/
  ├── 07_Model_Comparison/
...
PATH_ROI_PAUSA = os.path.join(BASE, "feature_table_roi_ROI_wide_2_6_depth_1_2.csv")
PATH_ROI_STD   = os.path.join(BASE, "feature_table_roi_STD_ROI_wide_2_6_depth_1_2.csv")
PATH_MECH      = r"C:\Users\tomga\Desktop\Lab Data science\Dataset_giusti\mechanical_properties\mechanical_properties_summary.csv"

SCRIPT_DIR  = os.path.dirname(os.path.abspath(__file__))
ROOT_OUT    = os.path.join(SCRIPT_DIR, "Predict_ROI_2_6_1_2_All")
...
# ══════════════════════════════════════════════════════════════════════════════
# 1. LOAD DATA & PCA  (fitted on paused only, applied to all)
# ══════════════════════════════════════════════════════════════════════════════
...
def compute_ols_per_specimen(row):
    t_cols = sorted([c for c in row.index if c.startswith("T") and c[1:].isdigit()],
...
rows_p = [{"specimen": r["specimen"], "group": f"{int(r['pausa_s'])}s", "pausa_s": int(r["pausa_s"]),
           "temp_0": r["T0"], "temp_1": r["T1"], "temp_2": r["T2"], "temp_3": r["T3"],
           "temp_4": r["T4"], "temp_5": r["T5"], "temp_6": r["T6"],
           "heating_rate": r["heating_rate_t0"], "roi_std": r["roi_std_spaziale_mean"],
           "bed_temp": r["bed_temp_C"]} for _, r in df_p.iterrows()]

rows_s = [{"specimen": r["specimen"], "group": "STD", "pausa_s": 0,
           "temp_0": r["intercept_OLS"], "temp_1": r["T1"], "temp_2": r["T2"], "temp_3": r["T3"],
           "temp_4": r["T4"], "temp_5": r["T5"], "temp_6": r["T6"],
           "heating_rate": r["heating_rate_t0"], "roi_std": r["roi_std_spaziale_mean"],
           "bed_temp": r["intercept_OLS"]} for _, r in df_s.iterrows()]
...
gpr_kernel = C(1.0, (1e-3, 1e3)) * RBF(length_scale=1.0, length_scale_bounds=(1e-2, 10.0)) \
...
for train_idx, test_idx in loo.split(X_pc1):
    ...
    poly_pipe = Pipeline([("poly", PolynomialFeatures(degree=2, include_bias=True)),
                          ("lr",   LinearRegression())])
    ...
    gpr = GaussianProcessRegressor(kernel=gpr_kernel, n_restarts_optimizer=5,
...
# Create fully trained estimators for surface plotting
m1_est = LinearRegression().fit(X_pc1, y)
m2_est = LinearRegression().fit(X_pc12, y)
m3_est = Pipeline([("poly", PolynomialFeatures(degree=2, include_bias=True)),
                   ("lr",   LinearRegression())]).fit(X_pc1, y)
m4_est = Pipeline([("scaler", StandardScaler()),
                   ("svr", SVR(kernel="rbf", C=1.0, epsilon=0.1, gamma="scale"))]).fit(X_pc12, y)
m5_est = Pipeline([("scaler", StandardScaler()),
                   ("gpr", GaussianProcessRegressor(kernel=gpr_kernel, n_restarts_optimizer=5,
                                                    normalize_y=True, random_state=42))]).fit(X_pc12, y)
...
# 3. SAVE SUMMARY TXT
# ══════════════════════════════════════════════════════════════════════════════
with open(os.path.join(DIRS["sum"], "pca_prediction_summary.txt"), "w") as f:
...
scatter = ax.scatter(df_all["PC1"], df_all["PC2"], c=df_all["peso_rottura_kg"],
                     cmap="viridis", vmin=y.min(), vmax=y.max(), s=150, zorder=3, edgecolors="#333", linewidths=1)
plt.colorbar(scatter, ax=ax).set_label("Breaking Load (kg)", fontsize=12)

df_std = df_all[df_all["group"] == "STD"]
ax.scatter(df_std["PC1"], df_std["PC2"], c=df_std["peso_rottura_kg"], cmap="viridis",
...
for _, row in df_all.iterrows():
    ...
    ax.annotate(row["specimen"].replace("Rec-","").replace("_","\n"),
                (row["PC1"], row["PC2"]), textcoords="offset points", xytext=(6,5), fontsize=8, fontweight=fw)
    
ax.axhline(0, color="#bbb", linewidth=0.8); ax.axvline(0, color="#bbb", linewidth=0.8)
...
plt.close()
print(f"Saved: {DIRS['pca']}/pca_breaking_scatter.png")


# ══════════════════════════════════════════════════════════════════════════════
# 5. PLOTS — Per-Model (Prediction vs Actual, Residuals)
# ══════════════════════════════════════════════════════════════════════════════
...
for k, m in models_data.items():
    ...
    fig.patch.set_facecolor("white"); ax.set_facecolor("white")
    for _, row in df_all.iterrows():
        ax.scatter(row["peso_rottura_kg"], row[f"pred_{k}"], color=get_color(row), marker=get_marker(row), s=150 if row["group"]=="STD" else 120, edgecolors="white", zorder=3)
    ...
    ax.plot([min_val, max_val], [min_val, max_val], color="#333", linestyle="--", linewidth=1.5,
            label="Perfect prediction")
    ax.set_xlabel("Actual Breaking Load (kg)", fontsize=12)
    ax.set_ylabel("Predicted (LOOCV) kg", fontsize=12)
    ax.set_title(f"LOOCV Prediction vs Actual — {m['name']}\nRMSE={m['rmse']:.3f} kg, $R^2$={m['r2']:.3f}",
                 fontsize=13, pad=10)
    ax.legend(handles=legend_handles + [plt.Line2D([0],[0],color='#333',linestyle='--')], labels=[h.get_label() for h in legend_handles] + ['Perfect prediction'], fontsize=10)
    ax.grid(True, linestyle="--", alpha=0.4)
    ax.set_xlim(min_val, max_val); ax.set_ylim(min_val, max_val)
    fig.savefig(os.path.join(m['dir'], f"prediction_vs_actual.png"), dpi=150, bbox_inches="tight")
    plt.close()
    ...
    fig.patch.set_facecolor("white"); ax.set_facecolor("white")
    ax.bar(x_pos, df_all_sorted[f"resid_{k}"].values,
           color=[get_color(r) for _, r in df_all_sorted.iterrows()], edgecolor="white")
    ax.axhline(0, color="#333", linewidth=1.5)
    ax.set_xticks(x_pos)
    ax.set_xticklabels(df_all_sorted["specimen"].str.replace("Rec-","").str.replace("_","\n"),
    ...
    fig.savefig(os.path.join(m['dir'], f"loocv_residuals.png"), dpi=150, bbox_inches="tight")
    plt.close()
    
    print(f"Saved standard plots in: {m['dir']}")

# Specific plot for Model 1: Regression Line on PC1
fig, ax = plt.subplots(figsize=(8, 6))
fig.patch.set_facecolor("white"); ax.set_facecolor("white")
for _, row in df_all.iterrows():
    ax.scatter(row["PC1"], row["peso_rottura_kg"], color=get_color(row), marker=get_marker(row), s=150 if row["group"]=="STD" else 120, edgecolors="white", zorder=3)
...
plt.close()
print(f"Saved: {DIRS['m1']}/pc1_vs_breaking_regression.png")


# ══════════════════════════════════════════════════════════════════════════════
# 6. PLOTS — Model Comparison
# ══════════════════════════════════════════════════════════════════════════════
...
for ax_, vals, ylabel, title in [
    ...
    ax_.set_ylabel(ylabel, fontsize=12); ax_.set_title(title, fontsize=13)
...
plt.close()
print(f"Saved: {DIRS['comp']}/model_comparison.png")

# ══════════════════════════════════════════════════════════════════════════════
# 7. GENERATE 3D & 2D PREDICTION SURFACES FOR ALL MODELS
#    + GPR CONFIDENCE SURFACES (3D + 2D)
# ══════════════════════════════════════════════════════════════════════════════
...
# Helper function to predict grid
...
for k, m in models_data.items():
    ...
    # Default model prediction surface
    Z_pred = get_grid_predictions(est, feat_type)

    # -------------------------------------------------------------------------
    # GPR special handling: mean + confidence interval
    # -------------------------------------------------------------------------
    if k == "m5":
        gpr_pipeline = m5_est
    ...
    if k != "m5":
        ...
        # GPR 3D plot with confidence surfaces
        fig = plt.figure(figsize=(12, 8))
        ax = fig.add_subplot(111, projection='3d')
        fig.patch.set_facecolor("white"); ax.set_facecolor("white")
        ...
        surf_low = ax.plot_surface(
            PC1_grid, PC2_grid, Z_low,
            cmap='Blues', alpha=0.22,
            linewidth=0, antialiased=True
        )

        surf_high = ax.plot_surface(
            PC1_grid, PC2_grid, Z_high,
            cmap='Reds', alpha=0.22,
            linewidth=0, antialiased=True
        )
    ...
    if k != "m5":
        ...
        # Add confidence contours to the current 2D GPR plot
        ax.contour(
            PC1_grid, PC2_grid, Z_low,
